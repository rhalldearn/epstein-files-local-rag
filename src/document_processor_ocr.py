...
class DocumentProcessorOCR:
    ...
    def __init__(self, chunk_size: int = 512, overlap: int = 50, use_ocr: bool = True, checkpoint_interval: int = 100):
        """
    ...
    def process_all_documents(self, base_dir: Path, output_file: Path, resume: bool = True) -> List[Dict[str, Any]]:
        ...
        if resume and processed_files_set:
            ...
            if (stored_params.get("chunk_size") != self.chunk_size or
                stored_params.get("overlap") != self.overlap or
                stored_params.get("use_ocr") != self.use_ocr):
